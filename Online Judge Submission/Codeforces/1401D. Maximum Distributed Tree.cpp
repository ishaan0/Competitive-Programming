...
int solve(){
    ...
    cin>> m ;
    vector<ll> fact(m) ;
    foreach(fact) cin>> val ;
    for(int i=m; i<n-1; i++) fact.pb(1) ;
    sort(rall(fact)) ;
    ...
    if(sz(fact) == n-1){
        for(int i=0; i<n-1; i++)
            ans = modAdd(ans, modMul(fact[i],freq[i])) ;
    }
    else{
        ll z = sz(fact) - (n-1) ;
        for(int i=0,j=0; i<sz(fact); i++){
            if(i<z) fact[i+1] = modMul(fact[i], fact[i+1]) ;
            else ans = modAdd(ans, modMul(fact[i], freq[j++])) ;
        }
    }
