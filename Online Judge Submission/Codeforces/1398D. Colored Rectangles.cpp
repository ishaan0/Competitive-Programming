...
ll func(int i,int j,int k){
    ...
    ans = max(ans, func(i+1,j+1,k)+(v1[i]*v2[j])) ;
    ans = max(ans, func(i+1,j,k+1)+(v1[i]*v3[k])) ;
    ans = max(ans, func(i,j+1,k+1)+(v3[k]*v2[j])) ;
...
int solve(){
    ...
    for(int i=0; i<g; i++) cin>> v2[i] ;
    for(int i=0; i<b; i++) cin>> v3[i] ;

    sort(v1,v1+r,greater<ll>()) ;
    sort(v2,v2+g,greater<ll>()) ;
    sort(v3,v3+b,greater<ll>()) ;
