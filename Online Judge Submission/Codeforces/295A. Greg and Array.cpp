...
int solve(){
    ...
    vector<ll> arr(n+5), line_q(m+5), line_v(n+5) ;
    for(int i=1; i<=n; i++)
        cin>> arr[i] ;

    vector<pair<pii, ll > > query(m+5) ;
    for(int i=1; i<=m; i++)
        cin>> query[i].ff.ff >> query[i].ff.ss >> query[i].ss ;
    ...
    for(int i=1; i<=m; i++){
        line_q[i] += line_q[i-1] ;
        if(line_q[i]){
            line_v[ query[i].ff.ff ] += ( line_q[i]*query[i].ss )  ;
            line_v[ query[i].ff.ss + 1 ] -= ( line_q[i]*query[i].ss )  ;
        }
    }
