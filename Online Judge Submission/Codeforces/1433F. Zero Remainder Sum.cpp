...
int n, m, k ;
int arr[lim][lim], dp[lim][lim][37][lim];

int func(int i,int j,int cnt,int sum){
    if(i==n){
        if(sum == 0) return 0 ;
        return -1e9 ;
    }
    int &ans = dp[i][j][cnt][sum] ;
    if(ans != -1) return ans ;
    ...
    if(cnt+1 <= m/2){
        if(j+1 < m)
            ans = max(ans, arr[i][j] + func(i,j+1,cnt+1,(sum+arr[i][j])%k)) ;
        ans = max(ans, arr[i][j] + func(i+1,0,0,(sum+arr[i][j])%k)) ;
    }
    /// Without taking current value
    if(j+1 < m)
        ans = max(ans, func(i,j+1,cnt,sum)) ;
    ans = max(ans, func(i+1,0,0,sum)) ;
