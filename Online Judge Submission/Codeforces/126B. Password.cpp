...
const ll m1 = 1423165193 ;
const ll b1 = 61 ;

vector<ll> arr, hash_val ;
string str ;
ll n ;

bool func(ll idx){
    ll len = arr[idx], h1 = 0, base_pow = 1 ;
    if(n-len < 2) return false ;

    for(int i=1,j=1; j<len; i++,j++){
        h1 = ((h1 * b1)%m1 + str[i]) % m1 ;
        base_pow = (base_pow * b1) % m1 ;
    }
    
    for(int i=len; i<n-1; i++){
        h1 = ((h1 * b1)%m1 + str[i]) % m1 ;
        if(h1 == hash_val[idx]) return true ;
        h1 = h1 - (base_pow * str[i-len+1])%m1 ;
        h1 = (h1 + m1) % m1 ;
    }
...
int solve(){
    ...
    ll h1 = 0 , h2 = 0, base_pow = 1 ;
    for(int i=0,j=n-1,k=1; i<n; i++,j--,k++){
        h1 = (h1 * b1 + str[i]) % m1 ;
        h2 = (h2  + str[j] * base_pow) % m1 ;
    ...
    if(sz(arr) == 0) return cout<< "Just a legend\n" , 0 ;
    ...
    if(func(lo)){
        for(int i=0; i<arr[lo]; i++) cout<< str[i] ;
        cout<< "\n" ;
    }
