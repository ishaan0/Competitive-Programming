...
int func(int i,int prev,int f){
    if(i==n) return 0 ;
    if(dp.count(mp(i,mp(prev,f)))) return dp[mp(i,mp(prev,f))] ;
    int ans = 0 ;

    if(arr[i] > prev){
         amax(ans, func(i+1, arr[i], f)+1) ;
         if(!f)
            amax(ans, func(i+1, prev+1, 1)+1) ;
     }
    else if(!f) amax(ans, func(i+1, prev+1, 1)+1) ;

    dp[mp(i,mp(prev,f))] = ans ;
    return ans ;
}
