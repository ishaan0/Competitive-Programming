...
int solve(){
    ...
    for(int i=0,j=0; j<5&&i<sz(pos); i++,j++)
        mx.pb(pos[i]) ;
    for(int i=0,j=0; j<5&&i<sz(neg); i++,j++)
        mx.pb(neg[i]) ;

    for(int i=sz(pos)-1,j=0; j<5&&i>=5; i--,j++)
        mx.pb(pos[i]) ;
    for(int i=sz(neg)-1,j=0; j<5&&i>=5; i--,j++)
        mx.pb(neg[i]) ;

    ll ans = -1e18,lim1 = sz(mx) ;
    for(int a = 0; a<lim1; a++){
        for(int b=a+1; b<lim1; b++){
            for(int c=b+1; c<lim1; c++){
                for(int d=c+1; d<lim1; d++){
                    for(int e=d+1; e<lim1; e++){
                        ll tmp = mx[a]*mx[b]*mx[c]*mx[d]*mx[e] ;
                        ans = max(tmp, ans) ;
                    }
                }
            }
