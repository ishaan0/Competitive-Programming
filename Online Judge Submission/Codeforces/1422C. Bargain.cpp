...
int solve(){
    ...
    ll ans = 0 , ten = 1 , suf = 0 ;
    for(ll i=n; i>=1; i--){
        ans = modAdd(ans, modMul(dig(str[i]), suf)) ;
        ans = modAdd(ans, modMul((i*(i-1))/2LL, modMul(dig(str[i]), ten))) ;
        suf = modAdd(suf, modMul(ten, n-i+1)) ;
        ten = modMul(ten, 10LL) ;
    }
