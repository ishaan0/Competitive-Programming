...
int query(int node,int lo,int hi,int qi,int qj,ll val){
    if(qj<lo || qi>hi) return 0 ;
    if(qi<=lo && hi<=qj){
        int upr = upper_bound(all(tree[node]),val) - tree[node].begin() ;
        int lwr = lower_bound(all(tree[node]),val) - tree[node].begin() ;
        return (upr-lwr) ;
    }
    int mid = lo + (hi-lo)/2 ;
...
void solve(){
    ...
    for(int i=1; i<=n; i++){
        ...
        ll val = arr[i]/k ;
        ll a = 0 ;
        if(i!=1) a = query(1,1,n,1,i-1,val) ;

        val = arr[i]*k ;
        ll b = 0 ;
        if(b!=n) b = query(1,1,n,i+1,n,val) ;
