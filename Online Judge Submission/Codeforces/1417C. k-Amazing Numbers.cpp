...
int solve(){
    ...
    for(int i=1; i<=n; i++){
        int mx = 0 ;
        if(sz(adj[i]) >= 1) mx = adj[i][0] ;
        for(int j=1; j<sz(adj[i]); j++)
            mx = max(mx, adj[i][j] - adj[i][j-1]) ;
        if(sz(adj[i]) >= 1) mx = max(mx, n - adj[i][sz(adj[i])-1]+1) ;
        ans[mx] = min(ans[mx], i) ;
    }
    ...
    for(int i=1; i<=n; i++){
        ans[i] = min(prev, ans[i]) ;
        if(ans[i] != 0) prev = ans[i] ;
