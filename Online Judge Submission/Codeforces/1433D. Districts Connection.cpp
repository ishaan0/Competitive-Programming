...
int solve(){
    ...
    if(sz(m) == 1) return cout<< "NO\n", 0 ;

    auto it = m.begin() ;
    int g1 = it->ff , id1 = it->ss ;
    it++ ;
    int g2 = it->ff, id2 = it->ss ;

    vector<pii > ans ;
    ans.pb(mp(id1, id2)) ;
    for(int i=0; i<n; i++){
        if(i+1 == id1 || i+1 == id2) continue ;
        if(arr[i] != g1) ans.pb(mp(i+1, id1)) ;
        else if(arr[i] != g2) ans.pb(mp(i+1,id2)) ;
    }

    cout<< "YES\n" ;
    foreach(ans) cout<< val.ff << " " << val.ss << "\n" ;
