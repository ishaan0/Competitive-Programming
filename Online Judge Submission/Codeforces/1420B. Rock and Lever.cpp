...
int solve(){
    ...
    int arr[n+1], pre[n+1][31], start[n+1][31];
    memz(pre) ;
    memz(start) ;
    ...
    for(int i=1; i<=n; i++){
        for(int j=0; j<30; j++)
            pre[i][j] = pre[i-1][j], start[i][j] = start[i-1][j] ;
        vector<int> bin ;
        ll val = arr[i] ;
        while(val != 0){
            bin.pb(val%2) ;
        ...
        for(int j=0; j<sz(bin); j++)
            pre[i][j] += bin[j] ;
        if(bin[sz(bin)-1]) start[i][sz(bin)-1]++ ;
    }

    ll ans = 0 ;
    for(int i=1; i<=n; i++){
        for(int j=29; j>=0; j--){
            if(pre[i][j]-pre[i-1][j] != 1) continue ;
            int one = pre[n][j] - pre[i][j] ;
            ans += min(one, start[n][j]-start[i][j]) ;
            break ;
        }
    }
