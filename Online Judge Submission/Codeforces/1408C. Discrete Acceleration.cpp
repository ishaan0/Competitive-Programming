...
ll n ;
double l ;
double arr[lim], tm1[lim], tm2[lim], sp1[lim], sp2[lim] ;

bool func(double t){
    double dis1 = -1, dis2 = -1 ;
    for(int i=1; i<=n+1; i++){
        if(t >= tm1[i-1] && t <= tm1[i]){
            double difft = t - tm1[i-1] ;
            dis1 = (difft * sp1[i-1] ) + arr[i-1] ;
            break ; 
        }
    }

    for(int i=n; i>=0; i--){
        if(t >= tm2[i+1] && t <= tm2[i]){
            double difft = t - tm2[i+1] ;
            dis2 = arr[i+1] - (difft * sp2[i+1] ) ;
            break ; 
        }
    }

    if(dis1==-1 || dis2==-1) return false ;
...
int solve(){
    ...
    for(int i=1; i<=n+1; i++, cur+=1){
        ...
        tm1[i] = tm1[i-1] + diff/cur ;
        sp1[i-1] = cur ;
    }

    cur = 1 ;
    for(int i=n; i>=0; i--, cur+=1){
        double diff = arr[i+1] - arr[i] ;
        tm2[i] = tm2[i+1] + diff/cur ;
        sp2[i+1] = cur ;
    }
