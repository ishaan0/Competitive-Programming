...
ll func(ll i,ll j,ll sum){
    ...
    if(pre[i][val] - pre[j][val] == 2) add = 2 ;
    else if(pre[i][val] - pre[j][val] > 2) add = 1 ;
    ans = min(ans, func(i+1,j, sum + add)) ;
    ans = min(ans, func(i+1,i-1,0)+ sum + k) ;
