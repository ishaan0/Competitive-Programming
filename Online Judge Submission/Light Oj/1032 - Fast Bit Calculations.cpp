...
ll func(ll small, ll prev, ll pos, ll couple){
    if(pos == sz(bits)) return couple ;
    ll &ans = dp[small][prev][pos][couple] ;
    if(ans != -1) return ans ;

    ans = 0 ;
    int lim = ((small) ? 1 : bits[pos]) ;

    for(int i=0; i<=lim; i++)
        ans += func(small | (i<bits[pos]), i, pos+1, (int)((i==1) && (prev==1)) + couple) ;
