...
void multiply(ll F[2][2], ll M[2][2]){
    ll a = ((F[0][0]*M[0][0])%sz + (F[0][1]*M[1][0])%sz)%sz ;
    ll b = ((F[0][0]*M[0][1])%sz + (F[0][1]*M[1][1])%sz)%sz ;
    ll c = ((F[1][0]*M[0][0])%sz + (F[1][1]*M[1][0])%sz)%sz ;
    ll d = ((F[1][0]*M[0][1])%sz + (F[1][1]*M[1][1])%sz)%sz ;
...
int main()
{
    ...
    while(t--){
        ...
        ll ans = ((fibbo(m+2)%sz)-(fibbo(n+1)%sz)+sz)%sz ;
        printf("%lld\n",ans) ;
    }
