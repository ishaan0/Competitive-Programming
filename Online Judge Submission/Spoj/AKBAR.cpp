...
int sz = 1000005 ;
vector<int> adj[1000005] , dist(1000005);
vector<bool> visited(1000005), gurded(1000005) ;
...
int main()
{
    ...
    while(t--){
        ...
        for(int i=0; i<m; i++){
            int sol,strn ;
            scanf("%d%d",&sol,&strn) ;
            soldiers.pb(mk(sol,strn)) ;
        }

        bool flag = true ;
        for(int i=0; i<soldiers.size(); i++){
            int sol = soldiers[i].ff ;
            int strn = soldiers[i].ss ;
            ...
            while(!q.empty()){
                ...
                q.pop() ;
                //cout<< "debug-> " << cur << endl ;
                if(strn < dist[cur]) break ;
                if(gurded[cur]){ flag = false ; break;}
